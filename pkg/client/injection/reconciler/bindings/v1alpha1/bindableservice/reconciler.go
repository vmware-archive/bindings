...
// controller reconciling v1alpha1.BindableService.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1alpha1.BindableService. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1alpha1.BindableService) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1alpha1.BindableService.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1alpha1.BindableService. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1alpha1.BindableService) reconciler.Event
}

// reconcilerImpl implements controller.Reconciler for v1alpha1.BindableService resources.
type reconcilerImpl struct {
	// Client is used to write back status updates.
	Client versioned.Interface

	// Listers index properties about resources
	Lister bindingsv1alpha1.BindableServiceLister
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister bindingsv1alpha1.BindableServiceLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	getter := r.Lister.BindableServices(namespace)
...
func (r *reconcilerImpl) updateStatus(existing *v1alpha1.BindableService, desired *v1alpha1.BindableService) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.BindingsV1alpha1().BindableServices(desired.Namespace)
		...
		updater := r.Client.BindingsV1alpha1().BindableServices(existing.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1alpha1.BindableService) (*v1alpha1.BindableService, error) {
	finalizerName := defaultFinalizerName

	getter := r.Lister.BindableServices(resource.Namespace)
	...
	patcher := r.Client.BindingsV1alpha1().BindableServices(resource.Namespace)
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1alpha1.BindableService) (*v1alpha1.BindableService, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1alpha1.BindableService, reconcileEvent reconciler.Event) (*v1alpha1.BindableService, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
