...
// controller reconciling v1alpha1.ServiceBinding.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1alpha1.ServiceBinding. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1alpha1.ServiceBinding) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1alpha1.ServiceBinding.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1alpha1.ServiceBinding. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1alpha1.ServiceBinding) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1alpha1.ServiceBinding if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1alpha1.ServiceBinding.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1alpha1.ServiceBinding) reconciler.Event
}

// ReadOnlyFinalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1alpha1.ServiceBinding if they want to process tombstoned resources
...
type ReadOnlyFinalizer interface {
	// ObserveFinalizeKind implements custom logic to observe the final state of v1alpha1.ServiceBinding.
	// This method should not write to the API.
	ObserveFinalizeKind(ctx context.Context, o *v1alpha1.ServiceBinding) reconciler.Event
}

// reconcilerImpl implements controller.Reconciler for v1alpha1.ServiceBinding resources.
type reconcilerImpl struct {
	...
	Lister servicev1alpha1.ServiceBindingLister
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister servicev1alpha1.ServiceBindingLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	getter := r.Lister.ServiceBindings(namespace)
...
func (r *reconcilerImpl) updateStatus(existing *v1alpha1.ServiceBinding, desired *v1alpha1.ServiceBinding) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.ServiceV1alpha1().ServiceBindings(desired.Namespace)
		...
		updater := r.Client.ServiceV1alpha1().ServiceBindings(existing.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1alpha1.ServiceBinding) (*v1alpha1.ServiceBinding, error) {

	getter := r.Lister.ServiceBindings(resource.Namespace)
	...
	patcher := r.Client.ServiceV1alpha1().ServiceBindings(resource.Namespace)
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1alpha1.ServiceBinding) (*v1alpha1.ServiceBinding, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1alpha1.ServiceBinding, reconcileEvent reconciler.Event) (*v1alpha1.ServiceBinding, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
